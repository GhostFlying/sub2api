...
7. If replay succeeds, push the sync branch and open or update a draft preview
   PR from `sync/upstream-<tag>` to `fork`. The PR body must contain the
   machine-readable `fork-sync-source` marker generated by the sync workflow,
   recording the source `fork`, source `upstream-release`, effective baseline,
   and generated head SHAs.
...
9. After review and passing checks, promote by commenting `/promote-fork` on the
   preview PR. Promotion must refuse the preview if any recorded source ref,
...
10. Push `sync/upstream-<tag>` and create or update the preview PR with a
    `fork-sync-source` marker containing the conflict issue's source `fork`,
    source `upstream-release`, effective baseline, and the repaired branch head.
11. Let the preview PR/check workflow run.
12. After review and green required checks, use `/promote-fork`; promotion is the

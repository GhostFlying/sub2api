...
func TestHandleStreamingResponse_StreamErrorBeforeOutput_ReturnsTypedErrorWithBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := newMinimalGatewayService()
	...
	require.JSONEq(t, upstreamBody, sseErr.RawData)
	require.Equal(t, "model unavailable for this account", ExtractUpstreamErrorMessage([]byte(sseErr.RawData)))
